...
<?php $this->registerJs("
    $('.delete-button-photo').click(function() {
        var detail = $(this).closest('.course-photo');
        var updateType = detail.find('.update-type');
        if (updateType.val() === " . json_encode(CoursePhoto::UPDATE_TYPE_UPDATE) . ") {
            //marking the row for deletion
            updateType.val(" . json_encode(CoursePhoto::UPDATE_TYPE_DELETE) . ");
            detail.hide();
        } else {
...
");
?>
    <div class="course-form tab-content">
                ...
                <div class="col-md-12">
                    ...
                    <div class="col-md-6">
                        ...
                        if ($model->isNewRecord) {
                            ...
                            /*  echo Html::img(Yii::$app->getHomeUrl() . 'uploads/course/' . $model->main_photo,
                            ...
                            echo $form->field($model, 'main_photo_file')->widget(FileInput::classname(), [
                                'options' => ['accept' => 'image/*'], 'pluginOptions' => [
                                    'showUpload' => false,
                                    'initialPreview' => [
                                        ["http://www.designineducation.com/backend/uploads/course/$model->main_photo"]
                                    ],
                    ...
                    <div class="col-md-6">
                        <div id="photo" class="tab-pane fade in active">
                                ...
                                <?php foreach ($modelDetails as $i => $modelDetail) : ?>
                                    <div class="row course-photo course-photo-<?= $i ?>">
                                        <div class="col-md-10">
                                            <?= Html::activeHiddenInput($modelDetail, "[$i]id") ?>
                                            <?= Html::activeHiddenInput($modelDetail, "[$i]updateType", ['class' => 'update-type']) ?>

                                            <?php
                                            echo $form->field($modelDetail, "[$i]course_photo")->widget(FileInput::classname(), [
                                                'options' => ['accept' => 'image/*'], 'pluginOptions' => [
                                                    'showUpload' => false,
                                                    'initialPreview' => [
                                                        ["http://www.designineducation.com/backend/uploads/course/related_photo/$modelDetail->photo_url"]
                                                    ],
                                                    'initialPreviewAsData' => true,
                                        ...
                                        <div class="col-md-2">
                                            <?= Html::button('x', ['class' => 'delete-button-photo btn btn-danger', 'data-target' => "course-photo-$i"]) ?>
                                        </div>
                                    </div>
                                <?php endforeach; ?>
