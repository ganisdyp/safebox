...
<?php $this->registerJs("
    $('.delete-button-photo').click(function() {
        ...
        var detail = $(this).closest('.showcase-owner');
        var updateType = detail.find('.update-type');
        if (updateType.val() === " . json_encode(ShowcaseOwner::UPDATE_TYPE_UPDATE) . ") {
            //marking the row for deletion
            updateType.val(" . json_encode(ShowcaseOwner::UPDATE_TYPE_DELETE) . ");
            detail.hide();
        } else {
            //if the row is a new row, delete the row
            detail.remove();
...
<div class="showcase-form">
                    ...
                    <div id="image-box" class="col-md-12">
                        ...
                        echo $form->field($model, 'main_photo_file')->widget(FileInput::classname(), [
                            'options' => ['accept' => 'image/*'], 'pluginOptions' => [
                                'showUpload' => false,
                                'initialPreview' => [
                                    ["http://www.designineducation.com/backend/uploads/showcase/$model->main_photo"]
                                ],
        ...
        <div class="tab-content col-md-12">
            <div id="photo" class="tab-pane fade in active">
                ...
                <?php foreach ($modelDetails as $i => $modelDetail) : ?>
                    <div class="showcase-profile showcase-profile-<?= $i ?>" style="margin-top: .75rem;">
                        <div class="col-md-5 col-xs-10">
                            <?= Html::activeHiddenInput($modelDetail, "[$i]id") ?>
                            <?= Html::activeHiddenInput($modelDetail, "[$i]updateType", ['class' => 'update-type']) ?>
                            <?php //echo $form->field($modelDetail, "[$i]showcase_photo")->fileInput()
                            //
                            echo $form->field($modelDetail, "[$i]showcase_photo")->widget(FileInput::classname(), [
                                'options' => ['accept' => 'image/*'], 'pluginOptions' => [
                                    'showUpload' => false,
                                    'initialPreview' => [
                                        ["http://www.designineducation.com/backend/uploads/showcase/related_photo/$modelDetail->showcase_url"]
                                    ],
                                    'initialPreviewAsData' => true,
                                    'initialCaption' => "$modelDetail->showcase_url",
                                    'initialPreviewConfig' =>[
                                        ['caption' => $modelDetail->showcase_url]
                                    ],
                                    'overwriteInitial' => false,
                                    'maxFileSize' => 100000
                                ],
                        ...
                        <div class="col-md-1 col-xs-2">
                            ...
                            <?= Html::button('x', ['class' => 'delete-button-photo btn btn-danger', 'data-target' => "showcase-profile-$i"]) ?>
                            </div>
                        </div>
                    </div>
                <?php endforeach; ?>
            ...
            <div id="student" class="tab-pane fade">
                <div class="col-md-12">
                    ...
                    <?php foreach ($modelDetails2 as $i => $modelDetail2) : ?>
                        <div class="row showcase-owner showcase-owner-<?= $i ?>">
                            <div class="col-md-12">
                                ...
                                <div class="col-md-6 text-right">
                                    <?= Html::button('x', ['class' => 'delete-button-owner btn btn-danger', 'data-target' => "showcase-owner-$i"]) ?>
                                </div>
                            </div>
                            <div class="col-md-12">
  ...
  $(\'#media-selector\').change(function(){
   ...
   $(\'#image-box\').hide();
   $(\'#video-box\').show();
     $(\'#showcase-media_type\').val(1);
    }else{
    
   $(\'#video-box\').hide();
   $(\'#image-box\').show();
    $(\'#showcase-media_type\').val(2);
    }
    });
